...
<a href="../courses/add" class="px-3 py-1 bg-[#d3a840] text-white rounded-md hover:opacity-25 transition mb-4 inline-block">Add Course</a>
<?php } ?>
                ...
                <?php foreach ($courses as $course) :?>
                    <tr class="hover:bg-[#634d17] transition">
                        ...
                        <?php if ($_SESSION["role"] === "admin" || ($_SESSION['role'] === "professor" && $course->professor_id === $professor->id)) { ?>
                            <a href="../courses/edit/<?= $course->id; ?>" class="px-3 py-1 bg-[#d3a840] text-white rounded-md hover:opacity-50 transition">Edit</a>
                            <a href="../courses/show/<?= $course->id; ?>" class="px-3 py-1 bg-[#a17d25] text-white rounded-md hover:opacity-50 transition">Show</a>
                        ...
                        <?php } elseif($_SESSION["role"] === 'student' && in_array($course->id,$studentCourses)  ) { ?>
                            <a href="../courses/enroll/<?= htmlspecialchars($course->id); ?>" class="px-3 py-1 bg-[#d3a840] text-white rounded-md hover:opacity-50 transition">enroll</a>
                        <?php } ?>
                        </td>
                    </tr>
                <?php endforeach; ?>
